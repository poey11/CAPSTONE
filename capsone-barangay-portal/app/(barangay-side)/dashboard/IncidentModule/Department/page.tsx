...
export default function Department() {
 ...
 const [highlightedId, setHighlightedId] = useState<string | null>(null);
  ...
  /*
      ...
      const dialogueSnap = await getDocs(collection(db, `IncidentReports/${incident.id}/DialogueMeeting`));
      setHasDialogue(!dialogueSnap.empty);

      const hearingSnap = await getDocs(collection(db, `IncidentReports/${incident.id}/GeneratedLetters`));
      setHasHearing(!hearingSnap.empty);
    };*/
 ...
 const openPopup = async (incident: any) => {
  ...
  const dialogueSnap = await getDocs(collection(db, `IncidentReports/${incident.id}/DialogueMeeting`));
if (!dialogueSnap.empty) {
  const firstDoc = dialogueSnap.docs[0].data();
  setDialogueMeetingData(firstDoc);
  setHasDialogue(true);
} else {
  ...
  const hearingSnap = await getDocs(collection(db, `IncidentReports/${incident.id}/SummonsMeeting`));
  setHasHearing(!hearingSnap.empty);
...
const openPopup = async (incident: any) => {
  ...
  // Get DialogueMeeting data
  const dialogueDocRef = doc(db, "IncidentReports", incident.id, "DialogueMeeting", incident.id);
  const dialogueDocSnap = await getDoc(dialogueDocRef);
  const dialogueData = dialogueDocSnap.exists() ? dialogueDocSnap.data() : null;

  // Get GeneratedLetters where letterType == "dialogue"
  const lettersSnap = await getDocs(
    query(
      collection(db, "IncidentReports", incident.id, "GeneratedLetters"),
      where("letterType", "==", "dialogue")
    )
  );

  const generatedDialogueLetter = !lettersSnap.empty ? lettersSnap.docs[0].data() : null;
