...
import ProgramsParticipationReportModal from "../../components/ProgramsParticipationReportModal";
...
import { generateProgramsMonthlyXlsx, generateProgramParticipationXlsx } from "./logic/programsReports";         
...
const ReportsPage = () => {
  ...
  const db = getFirestore();
    ...
    // Program Participation modal + loading
    const [showProgramsParticipationModal, setShowProgramsParticipationModal] = useState(false);
    const [loadingProgramsParticipation, setLoadingProgramsParticipation] = useState(false);
...
const handleGenerateProgramParticipationPDF = async ({
  programId,
}: {
  programId: string;
}) => {
  setLoadingProgramsParticipation(true);
  setIsGenerating(true);
  try {
    // 1) Generate XLSX in Storage
    const { fileUrl, programName } = await generateProgramParticipationXlsx({
      db,
      storage,
      programId,
    });

    // 2) Convert to PDF via your API
    const response = await fetch("/api/convertPDF", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fileUrl }),
    ...
    // 3) Download
    const blob = await response.blob();
    saveAs(blob, `Program_Participation_${programName}.pdf`);

    // 4) Notify PB
    try {
      await addDoc(collection(db, "BarangayNotifications"), {
        message: `A report (Program Participation Report) was generated by ${session?.user?.fullName || session?.user?.name || "Unknown"}.`,
        timestamp: new Date(),
        isRead: false,
        recipientRole: "Punong Barangay",
        transactionType: "System Report",
    ...
    // 5) Success popup/toast
    setPopupSuccessGenerateReportMessage("Program Participation Report generated successfully");
    setShowSuccessGenerateReportPopup(true);
    setTimeout(() => setShowSuccessGenerateReportPopup(false), 5000);
  } catch (err: any) {
    // Friendly error
    await showErrorToast?.("Failed to generate Program Participation PDF.");
  } finally {
    setLoadingProgramsParticipation(false);
    setShowProgramsParticipationModal(false);
    setIsGenerating(false);
  }
};
                ...
                <>
                  {currentPage === 1 && (
                    <div className="report-grid">
                      ...
                      <button
                        type="button"
                        onClick={() => setShowProgramsParticipationModal(true)}
                        disabled={loadingProgramsParticipation}
                        className={`report-tile ${loadingProgramsParticipation ? "disabled" : ""}`}
                        aria-busy={loadingProgramsParticipation}
                        aria-label="Generate Program Participation Report"
                      >
                        <img src="/Images/participation.png" alt="" className="report-icon" aria-hidden="true" />
                        <p className="report-title">
                          {loadingProgramsParticipation ? "Generating..." : "Program Participation Report"}
                        </p>
                      </button>

                      <ProgramsParticipationReportModal
                        open={showProgramsParticipationModal}
                        onClose={() => setShowProgramsParticipationModal(false)}
                        db={db}
                        loading={loadingProgramsParticipation}
                        onGenerate={(args) => {
                          void handleGenerateProgramParticipationPDF(args);
                        }}
                      />
