...
export default function EditResident() {
  ...
  const clusterOptions: Record<string, string[]> = {
    "East Fairview": ["Rina", "SAMAFA", "SAMAPLI", "SITIO KISLAP", "EFHAI"],
    "West Fairview": [
      "AUSTIN",
      "BASILIO 1",
      "DARISNAI",
  ...
  // 🔵 NEW: Field-change confirmation popup for Gender / DOB
  const [fieldConfirmOpen, setFieldConfirmOpen] = useState(false);
  const [pendingField, setPendingField] = useState<"" | "sex" | "dateOfBirth">("");
  ...
  // 🔵 NEW: Ask for confirmation before applying change to Gender or Date of Birth
  const requestFieldChange = (name: "sex" | "dateOfBirth", newValue: string) => {
    setPendingField(name);
    setPendingValue(newValue);
    setPreviousValue(String((formData as any)[name] || ""));
    setFieldConfirmOpen(true);
  };

  const confirmFieldChangeNo = () => {
    // Do not apply change; just close the dialog.
  ...
  const confirmFieldChangeYes = () => {
    if (!pendingField) return;

    if (pendingField === "sex") {
      setFormData((prev) => ({ ...prev, sex: pendingValue }));
      setFieldConfirmOpen(false);
    }

    if (pendingField === "dateOfBirth") {
      // Validate first
      const newDOB = pendingValue;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "sex" && typeof newValue === "string") {
      requestFieldChange("sex", newValue);
      return;
    }
  ...
  const handleSaveClick = async () => {
    const { firstName, lastName, address, generalLocation, cluster, dateOfBirth, age, sex, civilStatus, contactNumber, emailAddress } =
      formData;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const jobSeekerQuery = query(collection(db, "JobSeekerList"), where("residentId", "==", residentId));
      ...
      // --- Update ServiceRequests ---
      {
        ...
        const srQuery = query(collection(db, "ServiceRequests"), where("residentId", "==", residentId));
      ...
      // --- Update ProgramsParticipants ---
      {
        ...
        const ppQuery = query(collection(db, "ProgramsParticipants"), where("residentId", "==", residentId));
      ...
      const votersQuery = query(collection(db, "VotersList"), where("residentId", "==", residentId));
      ...
      const complainantQuery = query(collection(db, "IncidentReports"), where("complainant.residentId", "==", residentId));
      const respondentQuery = query(collection(db, "IncidentReports"), where("respondent.residentId", "==", residentId));

      const [complainantSnapshot, respondentSnapshot] = await Promise.all([getDocs(complainantQuery), getDocs(respondentQuery)]);
  ...
  return (
    <main className="add-resident-main-container">
                ...
                <>
                  <div className="add-main-resident-section-2-full-top">
                    <div className="add-main-resident-section-2-left-side">
                      ...
                      <div className="fields-section">
                        <p>
                          Date of Residency<span className="required">*</span>
                        </p>
                        <input
                          type="date"
                  ...
                  <div className="add-main-resident-section-2-full-bottom"></div>
                </>
              )}
                  ...
                  <div className="add-main-resident-others-mainsection">
                    <div className="add-main-resident-section-2-top-side">
                      <div className="box-container-outer-resclassification">
                        <div className="title-resclassification">Resident Classification</div>
                        ...
                        <div className="box-container-outer-pwdpic">
                          ...
                          <div className="box-container-pwdpic">
                            <div className={`file-upload-container-pwd ${invalidFields.includes("pwdIdFile") ? "pwd-error" : ""}`}>
                              <label htmlFor="pwd-id-file-upload" className="upload-link">
                                Click to Upload PWD ID
                              </label>
                              <input
                    ...
                    <div className="add-main-resident-section-2-bottom-side">
                            ...
                            {(identificationFile || identificationPreview) && (
                              <div className="identificationpic-display">
                                <div className="identification-picture">
                                  {identificationPreview && <img src={identificationPreview} alt="Preview" style={{ height: "200px" }} />}
                                </div>
                              </div>
                            )}
                            {(identificationFile || identificationPreview) && (
                      ...
                      <div className="box-container-outer-verificationdocs">
                        <div className="title-verificationdocs">Verification Documents</div>
                                ...
                                {verificationPreviews.map((preview, index) => (
                                  ...
                                  >
                                    <div key={index} className="identificationpic-file-name-image-display-indiv">
                                      ...
                                      <span>{verificationFiles[index]?.name || `Document ${index + 1}`}</span>
                                    </div>
                                  </a>
                                ))}
                              </div>
        ...
        <div className="confirmation-popup-overlay-add-resident">
          <div className="confirmation-popup-add-resident">
            ...
            <div className="yesno-container-add">
              ...
              <button onClick={confirmDiscard} className="yes-button-add">
                Yes
              </button>
            </div>
          </div>
      ...
      {showSavePopup && (
        <div className="confirmation-popup-overlay-add-resident">
          <div className="confirmation-popup-add-resident">
            ...
            <div className="yesno-container-add">
              <button onClick={() => setShowSavePopup(false)} className="no-button-add">
                No
              </button>
              <button onClick={confirmSave} className="yes-button-add">
                Yes
              </button>
            </div>
          </div>
      ...
      {/* 🔵 NEW: Field-change confirmation popup for Gender / Date of Birth */}
      {fieldConfirmOpen && (
        <div className="confirmation-popup-overlay-add-resident">
          <div className="confirmation-popup-add-resident">
            <img src="/Images/question.png" alt="warning icon" className="successful-icon-popup" />
            ...
            <h3 style={{ marginTop: 8 }}>
              {pendingField === "sex" ? "Gender" : pendingField === "dateOfBirth" ? "Date of Birth" : "Field"}
            </h3>
            <div style={{ marginTop: 8 }}>
              <div>
                <strong>From:</strong> <span>{previousValue || "—"}</span>
              </div>
              <div>
                <strong>To:</strong> <span>{pendingValue || "—"}</span>
              </div>
            </div>
            <div className="yesno-container-add" style={{ marginTop: 12 }}>
              <button onClick={confirmFieldChangeNo} className="no-button-add">
                No
              </button>
              <button onClick={confirmFieldChangeYes} className="yes-button-add">
                Yes
              </button>
            </div>
          </div>
