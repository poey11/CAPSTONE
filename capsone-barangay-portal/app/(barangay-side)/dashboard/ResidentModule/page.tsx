...
export default function ResidentModule() {
  ...
  // 🔽 New state for the "upload justification before edit" flow
  ...
  useEffect(() => {
    ...
    if (searchAddress) {
      filtered = filtered.filter((resident) => resident.address?.toLowerCase().includes(searchAddress.toLowerCase()));
    }

    if (searchOccupation) {
      filtered = filtered.filter((resident) => resident.occupation?.toLowerCase().includes(searchOccupation.toLowerCase()));
    }
    ...
    if (selectedLocation) {
      filtered = filtered.filter((resident) => resident.generalLocation?.toLowerCase().includes(selectedLocation.toLowerCase()));
    }
  ...
  // 🔽 UPDATED: When Edit is clicked, require a justification letter upload first
    ...
    try {
      ...
      for (const collectionName of collectionsToClean) {
        ...
        if (collectionName === "KasambahayList") {
          ...
          const q1 = query(collection(db, collectionName), where("residentId", "==", deleteUserId));
          const snapshot1 = await getDocs(q1);
          allDocs.push(...snapshot1.docs);

          // Get docs where employerId matches
          const q2 = query(collection(db, collectionName), where("employerId", "==", deleteUserId));
          const snapshot2 = await getDocs(q2);
          allDocs.push(...snapshot2.docs);
        } else {
          // delete residentId from other tables
          const q = query(collection(db, collectionName), where("residentId", "==", deleteUserId));
          const snapshot = await getDocs(q);
          allDocs.push(...snapshot.docs);
        }

        // Delete all matching docs
        const deletePromises = allDocs.map((docSnap) => deleteDoc(doc(db, collectionName, docSnap.id)));
        await Promise.all(deletePromises);
      }

      // delete the main resident
  ...
  /*
  CODE FOR INCIDENT RECORDS FOR THE VIEW PAGE
  */
  ...
  useEffect(() => {
    const fetchIncidentReports = async () => {
      ...
      const complainantQuery = query(incidentRef, where("complainant.residentId", "==", residentId));

      // Query where the resident is the respondent
      const respondentQuery = query(incidentRef, where("respondent.residentId", "==", residentId));

      // Query where the general residentId matches
      const generalQuery = query(incidentRef, where("residentId", "==", residentId));

      const [complainantSnap, respondentSnap, generalSnap] = await Promise.all([getDocs(complainantQuery), getDocs(respondentQuery), getDocs(generalQuery)]);

      const reports: any[] = [];

      complainantSnap.forEach((docx) => {
        reports.push({ id: docx.id, role: "Complainant", ...docx.data() });
      });

      respondentSnap.forEach((docx) => {
        if (!reports.find((r) => r.id === docx.id)) {
          reports.push({ id: docx.id, role: "Respondent", ...docx.data() });
        }
      });

      generalSnap.forEach((docx) => {
        if (!reports.find((r) => r.id === docx.id)) {
          reports.push({ id: docx.id, role: "Complainant", ...docx.data() });
        }
      });
  ...
  */
  ...
  useEffect(() => {
    const fetchServiceRequests = async () => {
      ...
      try {
        ...
        const requests: any[] = snapshot.docs.map((docu) => ({
          id: docu.id,
          ...docu.data(),
        }));
  ...
  // 🔽 Fetch list of edit justification letters for HISTORY tab
  useEffect(() => {
    const fetchJustifications = async () => {
      if (!residentId) return;
      try {
        const justRef = collection(db, "Residents", residentId, "EditJustifications");
        const q = query(justRef, orderBy("createdAt", "desc"));
        const snap = await getDocs(q);
        const items = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        setEditJustifications(items);
      } catch (e) {
        console.error("Error fetching edit justifications:", e);
      }
    };
    fetchJustifications();
  }, [residentId]);

  // 🔽 Helper for formatting Firestore timestamps or ISO strings
  ...
  const handleUploadAndContinue = async () => {
    ...
    try {
      setUploading(true);
      const path = `ResidentEditJustifications/${editResident.id}/${Date.now()}_${justificationFile.name}`;
      ...
      uploadTask.on(
        ...
        async () => {
          ...
          // Save metadata to Firestore under subcollection "EditJustifications"
          await addDoc(collection(db, "Residents", editResident.id, "EditJustifications"), {
            url,
          ...
          router.push(`/dashboard/ResidentModule/EditResident?id=${editResident.id}`);
        }
      );
    } catch (e) {
  ...
  return (
     ...
     */}
        ...
        <select className="resident-module-filter" value={showCount} onChange={(e) => setShowCount(Number(e.target.value))}>
          <option value="0">Show All</option>
          <option value="5">Show 5</option>
          <option value="10">Show 10</option>
        </select>

        <select className="resident-module-filter" value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)}>
                      ...
                      >
                        ...
                        <td>
                          <div className="residentmodule-actions">
                            ...
                            {!isAuthorized ? (
                              <>
                                <button className="residentmodule-action-edit hidden" /* edited this class */ aria-hidden="true">
                                  <img src="/Images/edit.png" alt="View" />
                                </button>
                                <button className="residentmodule-action-delete hidden" /* edited this class */ aria-hidden="true">
                    ...
                    <>
                      <div className="view-mainresident-content-left-side">
                        <div className="view-user-fields-section">
                          <p>Resident Number</p>
                          <input type="text" className="view-user-input-field" value={selectedUser.residentNumber || "N/A"} readOnly />
                        </div>
                        <div className="view-user-fields-section">
                          <p>Date of Residency</p>
                          <input type="text" className="view-user-input-field" value={selectedUser.dateOfResidency || "N/A"} readOnly />
                        </div>
                        <div className="view-user-fields-section">
                          <p>Address</p>
                          <input type="text" className="view-user-input-field" value={selectedUser.address || "N/A"} readOnly />
                        </div>
                        <div className="view-user-fields-section">
                          <p>Location</p>
                          <input type="text" className="view-user-input-field" value={selectedUser.generalLocation || "N/A"} readOnly />
                        </div>
                      </div>
                      <div className="view-mainresident-content-right-side">
                        ...
                        <div className="view-user-fields-section">
                          <p>Cluster</p>
                          <input type="text" className="view-user-input-field" value={selectedUser.cluster || "N/A"} readOnly />
                        </div>
                      </div>
                    </>
                  )}
                    ...
                    <>
                      <div className="mainresident-scroll">
                        <div className="view-main-user-content-left-side">
                          <div className="view-user-fields-section">
                            <p>Contact Number</p>
                            <input type="text" className="view-user-input-field" value={selectedUser.contactNumber || "N/A"} readOnly />
                          </div>
                          <div className="view-user-fields-section">
                            <p>Email Address</p>
                            <input type="text" className="view-user-input-field" value={selectedUser.emailAddress || "N/A"} readOnly />
                          </div>
                          <div className="view-user-fields-section">
                            <p>Civil Status</p>
                          ...
                          <div className="view-user-fields-section">
                            <p>Occupation</p>
                            <input type="text" className="view-user-input-field" value={selectedUser.occupation || "N/A"} readOnly />
                          </div>
                          <div className="view-user-fields-section">
                            <p>Precinct Number</p>
                            <input type="text" className="view-user-input-field" value={selectedUser.precinctNumber || "N/A"} readOnly />
                          </div>
                        </div>
                        <div className="view-main-user-content-right-side">
                          ...
                          <div className="view-user-fields-section">
                            <p>Place of Birth</p>
                            <input type="text" className="view-user-input-field" value={selectedUser.placeOfBirth || "N/A"} readOnly />
                          </div>
                          <div className="view-user-fields-section">
                            <p>Age</p>
                          ...
                          <div className="view-user-fields-section">
                            <p>Citizenship</p>
                            <input type="text" className="view-user-input-field" value={selectedUser.citizenship || "N/A"} readOnly />
                          </div>
                        </div>
                      </div>
                    </>
                      ...
                      <div className="others-main-section">
                        <div className="others-top-section">
                          <div className="view-main-user-content-left-side">
                            <div className="view-user-fields-section">
                              <p>Student</p>
                              <input type="text" className="view-user-input-field" value={selectedUser.isStudent ? "Yes" : "No"} readOnly />
                            </div>
                            <div className="view-user-fields-section">
                              <p>Senior Citizen</p>
                              <input type="text" className="view-user-input-field" value={selectedUser.isSeniorCitizen ? "Yes" : "No"} readOnly />
                            </div>
                          </div>
                          <div className="view-main-user-content-right-side">
                            <div className="view-user-fields-section">
                              <p>PWD</p>
                              <input type="text" className="view-user-input-field" value={selectedUser.isPWD ? "Yes" : "No"} readOnly />
                            </div>
                            <div className="view-user-fields-section">
                              <p>Solo Parent</p>
                              <input type="text" className="view-user-input-field" value={selectedUser.isSoloParent ? "Yes" : "No"} readOnly />
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="services-onlinereq-verification-requirements-section">
                            ...
                            <div className="services-onlinereq-verification-requirements-container">
                              {selectedUser.pwdIdFileURL ? (
                                <div className="file-name-image-display">
                                  <a href={selectedUser.pwdIdFileURL} target="_blank" rel="noopener noreferrer">
                                    <img
                            ...
                            <div style={{ marginTop: 10, textAlign: "center" }}>
                              ...
                              {selectedUser.pwdType === "Temporary" && (
                                <div>
                                  <strong>Valid Until:</strong> {selectedUser.pwdTemporaryUntil || "N/A"}
                                </div>
                              )}
                            </div>
                          </div>
                                ...
                                return (
                                  <tr key={incident.id} className="add-new-clickable-row">
                                    ...
                                    <td>
                                      <Link href={targetUrl}>
                                        {isOnline
                                          ...
                                          : incident.nature === "Others"
                                          ? incident.specifyNature
                                          : incident.nature}
                                      </Link>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                      <Link href={targetUrl}>
                                        <span
                                          className={`add-new-status-badge ${incident.status
                                            .toLowerCase()
                                            .replace(/[\s\-]+/g, "-")}`}
                                        >
                            ...
                            <tbody>
                              {serviceRequests.map((req) => (
                                <tr key={req.id} className="add-new-clickable-row">
                                  <td>
                                    <Link href={`/dashboard/ServicesModule/ViewRequest?id=${req.id}`}>{req.requestId}</Link>
                                  </td>
                                  <td>
                                    <Link href={`/dashboard/ServicesModule/ViewRequest?id=${req.id}`}>{req.purpose}</Link>
                                  </td>
                                  <td>
                                    <Link href={`/dashboard/ServicesModule/ViewRequest?id=${req.id}`}>{req.docType}</Link>
                                  </td>
                                  <td>
                                    <Link href={`/dashboard/ServicesModule/ViewRequest?id=${req.id}`}>
                                      <span
                                        className={`add-new-status-badge ${req.status?.toLowerCase().replace(/[\s\-]+/g, "-")}`}
                                      >
