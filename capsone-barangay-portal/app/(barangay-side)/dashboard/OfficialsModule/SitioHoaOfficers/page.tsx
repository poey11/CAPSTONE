...
interface NewOfficerDetails {
  ...
  facebook?: string;
  position?: string;
  otherPosition?: string;           // no longer used for “Others”, kept for backward compatibility
  location?: string;
...
export default function SitioHoaOfficersModule() {
  ...
  const [positionDropdown, setPositionDropdown] = useState("");   // kept for filter UI (table)
  ...
  // Start blank so Position shows placeholder until Department is chosen
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(officersCollection, (snapshot) => {
      const data: NewOfficerDetails[] = snapshot.docs.map((docSnap) => ({
        id: docSnap.id,
        ...
        clusterSection: docSnap.data().clusterSection,
  ...
  // Auto-fill Position from Department (and keep it disabled/read-only)
  useEffect(() => {
    setNewOfficerDetails(prev => {
      let autoPos = "";
      if (prev.department === "HOA") autoPos = "Association President";
      else if (prev.department === "SITIO") autoPos = "Sitio President";
      // Only update if changed
      return prev.position === autoPos ? prev : { ...prev, position: autoPos };
    });
  }, [newOfficerDetails.department]);
  ...
  const validateAndConfirm = () => {
    ...
    // Position is auto-set by Department; we don’t validate it directly anymore
  ...
  const addNewOfficer = async () => {
    try {
      ...
      const officerData: NewOfficerDetails = {
        ...newOfficerDetails,
        image: imageUrl,
      ...
      setPopupMessage("Officer created successfully!");
  ...
  const clusterOptions: Record<string, string[]> = {
    "East Fairview": [
      "RINA","SAMAFA","SAMAPLI","SITIO KISLAP","EFHAI","EFHNAI","RABOSNA HOA",
      "SAPAMANAI HOA","TOP JADE","UEHAI","SUNNY VILLAS HOA","SIRNAI HOA"
    ],
    "West Fairview": [
      "AUSTIN","BASILIO 1","DARISNAI","IRIS KALIWA","IRIS KANAN","JAGUAR","L.S.T.",
      "LOTUS","MAGNOLIA 1","MAGNOLIA 2","MALIBU","MUSTANG BENZ","MUSTANG LUPAMU",
      "MUSTANG RELESA","MUSTANG VILLE","NARCISSUS","OPEL","PAAS COMPOUND","Q LAWS",
      "Q MAGNOLIA","REGALADO","ROCES VILLAS","SITIO TIBAGAN","TARUNAI","TUPHA","ULNA",
      "UNITED FAIRLANE","URLINA","VERBENA 1","VERBENA 2","VERBENA 3","VERBENA 4","VERBENA 5",
      "ACHIEVERS HOA","BISTEKVILLE HOA","BRESCIA SUBD. HOA","COUNTRY HOME HOA","CUIDAD VERDE HOA",
      "DAIT HOA","DE VEGA HOA","FALCON HOA","HUNTER-HILLMAN HOA","IVORY EXECUTIVE VILLAGE",
      "LUPANG BIYAYA HOA","ORCHID VILLAS HOA","PITIMINI HOA","PONTIAC HOA","PRIMAVERA HOMES",
      "STO. NIÑO VILLAGE","VILLA ORION HOA","WEST FAIRVEW HOA","TULIP RESIDENCES HOA",
    ],
    "South Fairview": [
      "AKAP","ARNAI","BASILIO 2","BUICK EXT.","CUGON","F.L.N.A","FEWRANO","HILLMAN","LYRIC",
      "REREFNAI","SIKAP","BBCHAI HOA","BELCHA","BUICK HOA","COLT HOA","SAMAKA VILLAGE",
      "UPPER CORVETTE HOA","WINDSOR HOA",
    ]
  };
  ...
  return (
    <main className="officiers-main-container">
      ...
      <div className={`officers-section-2 ${filtersLoaded ? "filters-animated" : ""}`}>
        ...
        {/* Position filter for table (not the popup) */}
        <select
          className="officers-filter"
            ...
            <tbody>
              {currentUser.map((official) => (
                <tr key={official.id}>
                  ...
                  <td>{official.clusterSection === "Others" ? official.otherClusterSection : official.clusterSection}</td>
            ...
            <div className="add-officer-main-container">
              ...
              <div className="add-officer-info-main-container">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setNewOfficerDetails(prev => ({
                          ...
                          department: dept,
                          position:
                            dept === "HOA" ? "Association President" :
                            dept === "SITIO" ? "Sitio President" : ""
                        }));
                      }}
                    >
                ...
                <div className="add-officer-content-right-side">
                  {/* Position is NOT a dropdown anymore; auto-filled and disabled */}
                  <div className="fields-section">
                    <p>Position</p>
                    <input
                      type="text"
                      ...
                      placeholder="Position (auto-set by Department)"
                      readOnly
                      disabled
                    />
                  </div>
                  ...
                  <div className="fields-section">
                    ...
                    <select
                      ...
                      required
                      disabled={!newOfficerDetails.location}
                    >
