...
import {collection, doc, deleteDoc, onSnapshot, query, where, getDoc, getDocs, getCountFromServer , count, orderBy  } from "firebase/firestore";
...
const  getAllSpecificDocument =  (collect: string,  attribute: string, sign:any, value: string,setData:(data: any[])=>  void) => {
    try{
        const reportCollection = query(collection(db, collect), where(attribute, sign, value),orderBy("createdAt", "desc"));
